...
namespace TheCollectors.Content.Tiles.RefinedMeteoriteSet
{
	...
	internal class RefinedMeteoriteLamp : ModTile
	{
		...
		public override void SetStaticDefaults()
		{
			...
			if (!Main.dedServ)
			{
				flameTexture = ModContent.Request<Texture2D>("TheCollectors/Content/Tiles/RefinedMeteoriteSet/RefinedMeteoriteLamp_Flame"); // We could also reuse Main.FlameTexture[] textures, but using our own texture is nice.
			}
		}

		public override void KillMultiTile(int i, int j, int frameX, int frameY)
		{
			Item.NewItem(new EntitySource_TileBreak(i, j), i * 16, j * 16, 16, 48, ModContent.ItemType<Content.Items.Placeable.RefinedMeteoriteSet.RefinedMeteoriteLamp>());
		}
