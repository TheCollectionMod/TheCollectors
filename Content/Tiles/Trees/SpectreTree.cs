...
{
    public class SpectreTree : ModTree
    {
        ...
        {
            GrowsOnTileId = [ModContent.TileType<SpectreSoilTile>()];
            texture = ModContent.Request<Texture2D>("TheCollectors/Content/Tiles/Trees/SpectreTree");
            branchesTexture = ModContent.Request<Texture2D>("TheCollectors/Content/Tiles/Trees/SpectreTree_Branches");
            topsTexture = ModContent.Request<Texture2D>("TheCollectors/Content/Tiles/Trees/SpectreTree_Tops");
        }
        ...
        public override bool Shake(int x, int y, ref bool createLeaves)
        {
            switch (Main.rand.Next(3))
            {
                ...
                case 1:
                    Item.NewItem(WorldGen.GetItemSource_FromTreeShake(x, y), new Vector2(x, y) * 16, ModContent.ItemType<Items.Consumables.Food.SpectreFruit>());
                    break;

                default:
                    if (Main.netMode != NetmodeID.MultiplayerClient)
                    {
                        NPC.NewNPC(WorldGen.GetItemSource_FromTreeShake(x, y), x * 16, y * 16, ModContent.NPCType<NPCs.Critters.SpectreSquirrel>());
                    }
                    break;
            }
            return false;
