    ...
    class StardustTree : ModTree
	{
		...
		public override bool Shake(int x, int y, ref bool createLeaves)
		{
			...
			else if (effect == StardustTreeShakeEffect.Fruit)
			{
				...
				{
					Vector2 offset = this.GetRandomTreePosition(Main.tile[x, y]);
					Item.NewItem(WorldGen.GetItemSource_FromTreeShake(x, y), new Vector2(x, y) * 16 + offset, ModContent.ItemType<Items.Consumables.Food.StardustFruit>(), Main.rand.Next(1, 3));
				}
			}
