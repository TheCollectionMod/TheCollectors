    ...
    class HardenedMeteoriteTree : ModTree
	{
		...
		public override bool Shake(int x, int y, ref bool createLeaves)
		{
			...
			else if (effect == HardenedMeteoriteTreeShakeEffect.Fruit)
			{
				...
				{
					Vector2 offset = this.GetRandomTreePosition(Main.tile[x, y]);
					Item.NewItem(WorldGen.GetItemSource_FromTreeShake(x, y), new Vector2(x, y) * 16 + offset, Main.rand.NextBool() ? ModContent.ItemType<Content.Items.Consumables.Food.HardenedMeteoriteFruit>() : ModContent.ItemType<Content.Items.Consumables.Food.HardenedMeteoriteFruit>(), 1);
				}
			}
