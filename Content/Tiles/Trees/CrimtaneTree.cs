...
namespace TheCollectors.Content.Tiles.Trees
{
    class CrimtaneTree : ModTree
	{
		...
		{
			return ModContent.Request<Texture2D>("TheCollectors/Content/Tiles/Trees/CrimtaneTree");
		}       // This is the primary texture for the trunk. Branches and foliage use different settings.
		public override Asset<Texture2D> GetBranchTextures()
		{
			return ModContent.Request<Texture2D>("TheCollectors/Content/Tiles/Trees/CrimtaneTree_Branches");
		}       // Branch Textures
		public override Asset<Texture2D> GetTopTextures()
		{
			return ModContent.Request<Texture2D>("TheCollectors/Content/Tiles/Trees/CrimtaneTree_Tops");
		}       // Top Textures
		...
		public override bool Shake(int x, int y, ref bool createLeaves)
		{
			...
			else if (effect == CrimtaneTreeShakeEffect.NPC)
			{
				...
				npcType.Add(ModContent.NPCType<NPCs.Critters.CrimtaneSquirrel>(),1); 
			...
			else if (effect == CrimtaneTreeShakeEffect.Fruit)
			{
				...
				{
					Vector2 offset = this.GetRandomTreePosition(Main.tile[x, y]);
					Item.NewItem(WorldGen.GetItemSource_FromTreeShake(x, y), new Vector2(x, y) * 16 + offset,  ModContent.ItemType<Content.Items.Consumables.Food.CrimtaneFruit>(), Main.rand.Next(1, 3));
				}
			}
