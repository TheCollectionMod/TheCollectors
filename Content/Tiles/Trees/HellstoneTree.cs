...
namespace TheCollectors.Content.Tiles.Trees
{
    class HellstoneTree : ModTree
	{
		...
		{
			return ModContent.Request<Texture2D>("TheCollectors/Content/Tiles/Trees/HellstoneTree");
		}       // This is the primary texture for the trunk. Branches and foliage use different settings.
		public override Asset<Texture2D> GetBranchTextures()
		{
			return ModContent.Request<Texture2D>("TheCollectors/Content/Tiles/Trees/HellstoneTree_Branches");
		}       // Branch Textures
		public override Asset<Texture2D> GetTopTextures()
		{
			return ModContent.Request<Texture2D>("TheCollectors/Content/Tiles/Trees/HellstoneTree_Tops");
		}       // Top Textures
		...
		public override bool Shake(int x, int y, ref bool createLeaves)
		{
			...
			else if (effect == HellstoneTreeShakeEffect.Fruit)
			{
				...
				{
					Vector2 offset = this.GetRandomTreePosition(Main.tile[x, y]);
					Item.NewItem(WorldGen.GetItemSource_FromTreeShake(x, y), new Vector2(x, y) * 16 + offset, ModContent.ItemType<Items.Consumables.Food.HellstoneFruit>(), Main.rand.Next(1, 3));
				}
			}
