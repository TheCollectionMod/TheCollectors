...
namespace TheCollectors.Tiles.RefinedMeteoriteSet
{
	...
	internal class RefinedMeteoriteLantern : ModTile
	{
		...
		public override void SetStaticDefaults()
		{
			...
			if (!Main.dedServ)
			{
				flameTexture = ModContent.Request<Texture2D>("TheCollectors/Tiles/RefinedMeteoriteSet/RefinedMeteoriteLantern_Flame"); // We could also reuse Main.FlameTexture[] textures, but using our own texture is nice.
			}
		}

		public override void KillMultiTile(int i, int j, int frameX, int frameY)
		{
			Item.NewItem(new EntitySource_TileBreak(i, j), i * 16, j * 16, 16, 32, ModContent.ItemType<Items.Placeable.RefinedMeteoriteSet.RefinedMeteoriteLantern>());
		}
		...
		public override void PostDraw(int i, int j, SpriteBatch spriteBatch)
		{
			...
			for (int c = 0; c < 7; c++)
			{
				...
				spriteBatch.Draw(flameTexture.Value, new Vector2(i * 16 - (int)Main.screenPosition.X - (width - 16f) / 2f + shakeX, j * 16 - (int)Main.screenPosition.Y + offsetY + shakeY) + zero, new Rectangle(frameX, frameY, width, height), new Color(92, 28, 97, 0), 0f, default, 1f, effects, 0f);
			}
		}
	}
}