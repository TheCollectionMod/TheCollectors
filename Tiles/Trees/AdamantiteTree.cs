...
namespace TheCollectors.Tiles.Trees
{
    class AdamantiteTree : ModTree
	{
		...
		{
			return ModContent.Request<Texture2D>("TheCollectors/Tiles/Trees/AdamantiteTree");
		}       // This is the primary texture for the trunk. Branches and foliage use different settings.
		public override Asset<Texture2D> GetBranchTextures()
		{
			return ModContent.Request<Texture2D>("TheCollectors/Tiles/Trees/AdamantiteTree_Branches");
		}       // Branch Textures
		public override Asset<Texture2D> GetTopTextures()
		{
			return ModContent.Request<Texture2D>("TheCollectors/Tiles/Trees/AdamantiteTree_Tops");
		}       // Top Textures
		...
		public override bool Shake(int x, int y, ref bool createLeaves)
		{
			...
			else if (effect == AdamantiteTreeShakeEffect.Fruit)
			{
				...
				{
					Vector2 offset = this.GetRandomTreePosition(Main.tile[x, y]);
					Item.NewItem(WorldGen.GetItemSource_FromTreeShake(x, y), new Vector2(x, y) * 16 + offset, Main.rand.NextBool() ? ModContent.ItemType<Items.Consumables.Food.AdamantiteFruit>() : ModContent.ItemType<Items.Consumables.Food.AdamantiteFruit>(), 1);
				}
			}
