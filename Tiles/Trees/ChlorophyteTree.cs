    ...
    class ChlorophyteTree : ModTree
	{
		...
		public override bool Shake(int x, int y, ref bool createLeaves)
		{
			...
			else if (effect == ChlorophyteTreeShakeEffect.Fruit)
			{
				...
				{
					Vector2 offset = this.GetRandomTreePosition(Main.tile[x, y]);
					Item.NewItem(WorldGen.GetItemSource_FromTreeShake(x, y), new Vector2(x, y) * 16 + offset, Main.rand.NextBool() ? ModContent.ItemType<Items.Consumables.Food.ChlorophyteFruit>() : ModContent.ItemType<Items.Consumables.Food.ChlorophyteFruit>(), 1);
				}
			}
